...
public class PathfinderScript : MonoBehaviour
{
    ...
    void Start() 
    {
        ...
        Instantiate(pathfinder, spawnTile.transform.position , Quaternion.identity);
    ...
    void AvoidCollision() 
    {
        ...
        if(Physics.Raycast(left, pathfinder.transform.forward, out hit, detectionDistance)) 
            offset += Vector3.right;    
        else if(Physics.Raycast(right, pathfinder.transform.forward, out hit, detectionDistance))
            offset -= Vector3.left;  

        //Handle raycast collisions above and below and move out of way accordingly.
        if(Physics.Raycast(up, pathfinder.transform.forward, out hit, detectionDistance)) 
            offset -= Vector3.up;    
        else if(Physics.Raycast(down, pathfinder.transform.forward, out hit, detectionDistance))
            offset += Vector3.up;  
